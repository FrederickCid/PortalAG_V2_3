...
{
    public partial class SolicitudInformeNoPickeados
    {
        SfGrid<NoPickeadosModel> Grid;
        ...
        string UrlHistoricos = "Solicitud/NoPickeados";
        private List<NoPickeadosModel> NoPickeadosList = new List<NoPickeadosModel>();
        private List<string> ButtonsGrid = new List<string>() { "ExcelExport", "Todos los no Pickeados" };
        ...
        public async Task CargarDatos(string inicio, string fin)
        {
            try
            {
                ...
                var lista = await service.Formulario.HttpClientInstance.GetAsync($"api/v2/{UrlHistoricos}/{inicio}/{fin}");
                if (lista.IsSuccessStatusCode)
                {
                    NoPickeadosList = JsonConvert.DeserializeObject<List<NoPickeadosModel>>(await lista.Content.ReadAsStringAsync());
                    Loading = false;
                    snakBarCreation("Listo!", Defaults.Classes.Position.BottomStart, Severity.Success, 1000);
                }               
                else
                {
                    NoPickeadosList = new List<NoPickeadosModel>();
                    Loading = false;
        ...
        public async Task CargarDatosBoton()
        {
            try
            {
                Loading = true;
                service = new MainServices();
                var lista = await service.Formulario.HttpClientInstance.GetAsync($"api/v2/{UrlHistoricos}/%20/%20");
                if (lista.IsSuccessStatusCode)
                {
                    NoPickeadosList = JsonConvert.DeserializeObject<List<NoPickeadosModel>>(await lista.Content.ReadAsStringAsync());
                    Loading = false;
        ...
        public async Task ToolbarClick(Syncfusion.Blazor.Navigations.ClickEventArgs args)
        {
            ...
            if (args.Item.Id == "Grid_Todos los no Pickeados") 
            {
                string vacio = " ";
                await CargarDatosBoton();
            }
        }
