...
{
    public partial class SolicitudInformeNCPendientes
    {
        ...
        private List<ConsultaNCModel> NCPendientes = new List<ConsultaNCModel>();
       // private List<NCDetalleModel> Detalle = new List<NCDetalleModel>();
        ...
        public async Task CargarDatos(string inicio, string fin)
        {
            try
            {
                ...
                {
                    NCPendientes = JsonConvert.DeserializeObject<List<ConsultaNCModel>>(await lista.Content.ReadAsStringAsync());
                    Loading = false;
                    snakBarCreation("Listo!", Defaults.Classes.Position.BottomStart, Severity.Success, 1000);
                }
                else
                {
                    NCPendientes = new List<ConsultaNCModel>();
                    Loading = false;
